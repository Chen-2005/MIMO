...
- 用户可以看到自己创建的设计音色（无论是否发布）
- 其他用户发布的音色（`is_public=1`）也可见
- 未发布的克隆音色不在列表中返回（前端通过 localStorage 管理）

返回字段包含 `is_public`（`0`=仅自己可见，`1`=已公开发布），前端据此显示正确的发布状态。

**音色设计发布流程**：设计完成后 `is_public=0`，用户需先试听确认效果，满意后再点击发布按钮将 `is_public` 设为 `1`，此时音色才会对其他用户可见。
